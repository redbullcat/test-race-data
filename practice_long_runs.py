...
def show_practice_long_runs(df, team_colors):
    ...
    # --- Find longest no-pit stint per car across all sessions ---
    # Pits indicated by 'B' in CROSSING_FINISH_LINE_IN_PIT
    longest_runs = []

    for car_number, group in filtered_df.groupby("NUMBER"):
        # Sort by PRACTICE_SESSION and LAP_NUMBER so runs across sessions are considered properly
        ...
        for idx, row in group.iterrows():
            ...
            # Pit lap detected, skip this lap and next lap (out lap)
            ...
            # If starting new stint, mark session
