...
def show_driver_pace_chart(df, selected_cars, top_percent, selected_classes, team_colors):
    ...
    def filter_top_percent_laps(df, percent):
        filtered_dfs = []
        for (car_number, driver), group in df.groupby(["NUMBER", "DRIVER"]):
    ...
    avg_df = (
        filtered_df.groupby(["DRIVER", "TEAM", "NUMBER", "CLASS"], as_index=False)["LAP_TIME_SECONDS"]
        .mean()
        .sort_values("LAP_TIME_SECONDS", ascending=True)
    )

    # Assign colours based on team
    ...
    avg_df["Label"] = avg_df["DRIVER"] + " — " + avg_df["TEAM"] + " (" + avg_df["NUMBER"].astype(str) + ")"
