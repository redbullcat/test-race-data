...
def show_lap_position_chart(df, selected_cars, selected_classes, team_colors):
    ...
    max_position = df.groupby("LAP_NUMBER")["NUMBER"].nunique().max()

    lap_positions = {f'Lap {i}': [None] * max_position for i in range(1, max_lap + 1)}

    for lap in range(1, max_lap + 1):
        lap_df = df[df["LAP_NUMBER"] == lap].sort_values("ELAPSED").reset_index(drop=True)
    ...
    for team, color in team_colors.items():
        cars_in_team = df[df["TEAM"].str.lower() == team.lower()]["NUMBER"].unique()
    ...
    fig_lap.update_layout(
        title="Lap-by-Lap Position Chart",
